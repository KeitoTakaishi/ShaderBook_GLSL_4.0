...
void ofApp::setup() {
	...
	_lightPos = ofVec4f(ofVec3f(lightPos).x, ofVec3f(lightPos).y, ofVec3f(lightPos).z, 0.0);
	...
	memcpy(blockBuffer_light + offset[0], &_lightPos, 4 * sizeof(GLfloat));
	...
	memcpy(blockBuffer_material + offset[3], &_shininess, sizeof(GLfloat));
...
void ofApp::draw() {
	...
	_Ld = ofVec4f(ofVec3f(Ld).x, ofVec3f(Ld).y, ofVec3f(Ld).z, 0.0);
	_Ls = ofVec4f(ofVec3f(Ls).x, ofVec3f(Ls).y, ofVec3f(Ls).z, 0.0);
	glBufferSubData(GL_UNIFORM_BUFFER, 0 , 4 * sizeof(GLfloat), &_lightPos);
	...
	glBufferSubData(GL_UNIFORM_BUFFER, 0 + 52, 4 * sizeof(GLfloat), &_shininess);
